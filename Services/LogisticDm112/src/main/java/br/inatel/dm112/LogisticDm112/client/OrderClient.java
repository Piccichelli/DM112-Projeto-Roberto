package br.inatel.dm112.LogisticDm112.client;
...
public class OrderClient {
	 ...
	 */
	public List<Order> getOrdersByDeliveryManId(Integer deliveryManId) {
		String url = restURL + endpoint + "/delivery-man/" + deliveryManId;
		System.out.println("URL: " + url);
	...
	public void startOrderDelivery(int orderNumber) {
		String url = restURL + endpoint + "/" + orderNumber + "/startDelivery";

		makePUTCallWithEmptyBody(url);
	}

	public void confirmOrderDelivery(int orderNumber) {
		String url = restURL + endpoint + "/" + orderNumber + "/concludeDelivery";
	...
	private void makePUTCallWithEmptyBody(String url) {
		try {
			...
			if(response.statusCode() != HttpStatus.NO_CONTENT.ordinal()) {
				throw new RuntimeException("Error updating Order Delivery. statusCode" + response.statusCode());
			}
		} catch (Exception e) {
			e.printStackTrace();
